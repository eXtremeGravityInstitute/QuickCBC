...
int main(int argc, char *argv[])
{
  ...
  /*********************   With the data read in, we can now do the CBC PE *************************/
    ...
    MCMC_all(net, mxc, Nall, chainA, pallx, who, heat, historyall, global, freq, D, SN, SM, N, Tobs, ttrig, r);
...
void CBC_start(struct Net *net, int *mxc, FILE *chainS, double **paramx, double **skyx, double **pallx, int *who, double *heat, double ***history, double ***global, RealVector *freq, double **D, double **Dtime, double **SN, int N, double Tobs, gsl_rng *r)
{
    ...
    /*   // Here we can create global proposal, currently turned off and made uniform
     
     
     params[5] = Tobs/2.0;   // The maximization re-resets the reference time
     ...
     sprintf(command, "Mctc_%d_%d_%d.dat", (int)(Tobs), (int)ttrig, (int)(q));
     out = fopen(command,"w");
     for(j = 0; j < NM; j++)
     {
     lMc = lMcmin + (double)(j)*dlMc;
     ...
     log_likelihood_scan(net, D, params, freq, SN, Larray, Tarray, N, Tobs);
     
     for(i = -N/2; i < N/2; i++)
     {
     fprintf(out,"%e %e %e\n", (double)(i)*dt, exp(lMc), Larray[i]);
     global[k][j][i+N/2] = Larray[i];
     if(Larray[i] > x)
     {
     x = Larray[i];
     lMcx = params[0];
    ...
    for(k = 1; k <= NC; k++)
    {
    logL[k] = log_likelihood(net, D, paramx[k], freq, SN, N, Tobs);
     if(logL[k] > Lmax)
      {
        Lmax = logL[k];
        kk = k;
    ...
    for(j = 1; j <= NC; j++)
    {
        ...
        logLstart[j] = log_likelihood(net, D, paramx[j], freq, SN, N, Tobs);
       ...
       // printf("%f %f\n", SNRsq[0], SNRsq[1]);
        ...
        logL[j] = log_likelihood(net, D, paramx[j], freq, SN, N, Tobs);
                ...
                double ***history, double ***global, RealVector *freq, double **D, double **SN, int N, double Tobs, gsl_rng *r)
{
    ...
    for(k = 0; k < 5; k++)
    {
        ...
        for(j = 1; j <= NC; j++)
        {
            ...
            logL = log_likelihood(net, D, paramx[j], freq, SN, N, Tobs);
    ...
    if(lhold == 0)
       {
        if(lmax == 0)
        {
            logLx[k] = log_likelihood(net, D, paramx[k], freq, SN, N, Tobs);
        }
        else
        {
     ...
     for(k=1; k <= NC; k++)
     {
        ...
        if(mc%10 == 0)
        {
           ...
           // printf("%e %e\n", DL, exp(pallx[q][6])/(1.0e6*PC_SI));
            ...
            if(lmax == 1)
            {
                logL = log_likelihood(net, D, paramx[q], freq, SN, N, Tobs);
...
void updatei(int k, struct Net *net, int lmax, double *logLx, double **paramx, double **paramy, double *min, double *max, double *Fscale, int *who, double *heat, double ***history, double ***global, RealVector *freq, double **D, double **SN, double **ejump, double ***evec, int N, double Tobs, int **cv, int **av, gsl_rng *r)
{
        ...
        if(flag == 0)
        {
            ...
            if(lhold == 0)
            {
                if(lmax == 0)
                {
                    logLy = log_likelihood(net, D, paramy[q], freq, SN, N, Tobs);
                }
                else
                {
...
void MCMC_all(struct Net *net, int *mxc, int M, FILE *chain, double **paramx, int *who, double *heat, double ***history, double ***global, RealVector *freq, double **D, double **SN, double **SM, int N, double Tobs, double ttrig, gsl_rng *r)
{
    ...
    heterodyne(net, het, D, paramx[i], freq, SN, SM, N, Tobs);
    ...
    for(mc = -M/8; mc <= M; mc++)
    {
        // update the reference heterodyne occasionally in case solution wanders
        if(mc%10000==0 )
        {
         freehet(net,het);
         heterodyne(net, het, D, paramx[who[1]], freq, SN, SM, N, Tobs);
         for(k = 1; k <= NC; k++) logLx[k] = log_likelihood_het(net, het, paramx[k], Tobs);
        }
...
double log_likelihood_test(struct Net *net, struct Het *het, double **D, double *params, RealVector *freq, double **SN, double **SM, int N, double Tobs)
{
    ...
    MM = het->MM;
        ...
        for(i = 0; i < net->Nifo; i++)
        {
            HH = fourier_nwip(dh[i], dh[i], SN[i], 1, MM, N);
            HD = fourier_nwip(res[i], dh[i], SN[i], 1, MM, N);
            logL -= (HD+0.5*HH);
            printf("Test %d %f %f\n", i, HH, HD);
        }
...
void heterodyne(struct Net *net, struct Het *het, double **D, double *params, RealVector *freq, double **SN, double **SM, int N, double Tobs)
{
    ...
    for(id = 0; id < net->Nifo; id++)
    {
        HH = fourier_nwip(hb[id], hb[id], SN[id], 1, MM, N);
        HR = fourier_nwip(rb[id], hb[id], SN[id], 1, MM, N);
        logL += HR+0.5*HH;
    }
...
double log_likelihood_penalized(int ii, double *params, double *D, double *H, double *AU, double *PU, double *TDU, double *SN, int N, double Tobs, int pflag)
{
    ...
    if(lhold == 0)
    {
        ...
        for(i = imin; i < imax; i++)
        {
          ...
          cph = cos(ph);
          sph = sin(ph);
          twave[i] = norm*(H[i]*cph-H[N-i]*sph);
          twave[N-i] = norm*(H[N-i]*cph+H[i]*sph);
        }
     ...
     for (i = 0; i <= NU[k]; ++i)
        {
         ...
         AU[k][i] = HD/HH[k];
          ...
          LLU[k][i] = log_likelihood_penalized(i, params, D[k], h, AU[k], PU[k], TDU[k], SN[k], N, Tobs, pflag);
